...
def find_busiest_period(data):
  ...
  ## O(1) approach
  record_ts, cur_ppl, out_ts, max_ppl = None, 0, None, float('-inf')
  
  multiply = {0: -1, 1: 1}
  for record in data:
    cur_ts, delta_ppl, enter_or_not = record[0], record[1], record[2]

    if record_ts and cur_ts != record_ts:
      if cur_ppl > max_ppl:
        out_ts = record_ts
        max_ppl = max(max_ppl, cur_ppl)
        
      cur_ppl += multiply[enter_or_not] * delta_ppl
      record_ts = cur_ts
...
print(find_busiest_period(input))