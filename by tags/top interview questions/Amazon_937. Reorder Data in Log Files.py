...
class Solution:
    def reorderLogFiles(self, logs: List[str]) -> List[str]:
        digits=[i for i in logs if i.split()[1].isdigit()]
        letters= [i for i in logs if not i.split()[1].isdigit()]
        letters.sort(key= lambda item: list((item.split()[1:])+list(item.split()[0])))
